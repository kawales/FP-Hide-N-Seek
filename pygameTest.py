...
class Hider(pygame.sprite.Sprite):
    def __init__(self,activePhase=0, *groups: Group) -> None:
        super().__init__(*groups)
...
def menu():
    ...
    global props
    screen.fill((0,0,0))
    draw_text(screen,"HIDE N SEEK",30,600,300,(255,255,255))
    draw_text(screen,"Hider number:"+str(hiderNum),30,300,300,(255,255,255))
    pygame.draw.rect(screen,(0,255,0),pygame.Rect(500,350,300,75))
    for e in pygame.event.get():
        if(e.type==pygame.KEYDOWN and e.key==pygame.K_DOWN and hiderNum>1):
            hiderNum-=1;
        elif(e.type==pygame.KEYDOWN and e.key==pygame.K_UP and hiderNum<5):
            hiderNum+=1;
        if e.type == pygame.QUIT:
            pygame.quit()
        if e.type==pygame.KEYDOWN and  e.key == pygame.K_p:
            phase+=1
            for i in range(hiderNum):
                props.add(Hider(activePhase=i))

    draw_text(screen,"Press P to start",30,660,375)
    ...
    if(phase==0):
        ...
        for event in pygame.event.get():
            ...
            if event.type==pygame.KEYDOWN and  event.key == pygame.K_p:
                phase+=1
                GM.start()
    elif(phase>2 and phase<2+hiderNum*2+1 and phase%2!=0):
